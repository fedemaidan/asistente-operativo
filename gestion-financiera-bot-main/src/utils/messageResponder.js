...
const messageResponder = async (messageType, msg, sock, sender) => {
  switch (messageType) {
    ...
    case "image": {
      await FlowMapper.handleMessage(sender, msg, sock, "image");
    ...
    case "audio": {
      ...
      if (filePath) {
        await sock.sendMessage(sender, {
          text: `He recibido tu audio y lo he guardado en: ${filePath}`,
        });
      } else {
        await sock.sendMessage(sender, {
          text: "No pude guardar el audio. Intenta nuevamente.",
        });
      }
      break;
    }
    case "document": {
      await FlowMapper.handleMessage(sender, msg, sock, "document");
      break;
    }
    case "document-caption": {
      await FlowMapper.handleMessage(sender, msg, sock, "document-caption");
      break;
    }
    default: {
      await sock.sendMessage(sender, {
