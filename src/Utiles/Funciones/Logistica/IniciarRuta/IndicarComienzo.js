...
module.exports = async function IndicarComienzo(hojaRuta, sock) {
  try {
    ...
    if (Cliente?.Telefono) {
      ...
      console.error(
        "⚠️ No se pudo enviar mensaje al Cliente: Teléfono no disponible."
      );
    }
    ...
    if (Vendedor?.Telefono) {
      ...
      await enviarMensaje(
        Vendedor.Telefono + "@s.whatsapp.net",
        mensajeVendedor,
        sock
      );
    } else {
      console.error(
        "⚠️ No se pudo enviar mensaje al Vendedor: Teléfono no disponible."
      );
    }
    ...
    if (Chofer?.Telefono) {
      ...
      Detalles.forEach((detalle, index) => {
        mensajeChofer += `${index + 1}. 📍 *Dirección:* ${
          detalle.Direccion_Entrega || "No especificada"
      ...
      console.error(
        "⚠️ No se pudo enviar mensaje al Chofer: Teléfono no disponible."
      );
    }
